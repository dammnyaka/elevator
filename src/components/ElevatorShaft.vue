  ...
  <div class="shaft" :style="{ height: `${floorsNumber * floorHeight}px` }">
    <div class="elevator" :style="elevatorStyle">
      <div class="elevator_navigation">
        <span
          class="elevator_navigation-up"
          v-if="elevator.direction === 'up' && true"
        >
        </span>
        <span
          class="elevator_navigation-down"
          v-if="elevator.direction === 'down' && true"
        >
        </span>
        <span class="elevator_navigation-target">{{
...
export default {
  ...
  computed: {
    elevatorStyle() {
      ...
      let animationElevator =
        this.elevator.status === "resting" ? "name 1s infinite linear" : "";
      return {
        bottom: `${position}px`,
